<?php
    ...
    $query = 'update news set header = \''. $header . '\', body =\'  ' . $body . '\', email = \'' . $email . '\', url =  \'' . $url . '\', date =  \'' . date('l jS \of F Y h:i:s A') . '\' where id = '. $_POST['id'] . ';';//.  . '\');';
    echo $query;
    mysqli_query($db, $query) or die("\nОшибка при выполнении запроса ");
    mysqli_close($db);

    function validation($header, $body, $email){
        if(empty($header)){
